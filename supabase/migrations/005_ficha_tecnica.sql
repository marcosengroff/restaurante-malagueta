...
set quantidade = coalesce(quantidade, quantidade_utilizada),
...
add constraint itens_ficha_tecnica_quantidade_positive check (quantidade > 0);

alter table public.itens_ficha_tecnica
drop constraint if exists itens_ficha_tecnica_unidade_utilizada_check,
add constraint itens_ficha_tecnica_unidade_base_check check (unidade_base in ('g', 'ml', 'unidade'));
...
begin
  ...
  new.quantidade_utilizada = new.quantidade;
  new.custo_calculado = new.quantidade * ingrediente_custo;
...
begin
  ...
  set custo_total = coalesce((
    select sum(i.quantidade * ing.custo_unidade_base)
...
begin
  update public.itens_ficha_tecnica
  set custo_calculado = quantidade * new.custo_unidade_base,
      updated_at = now()
...
before insert or update of ingrediente_id, quantidade, unidade_base
