...
import { categoriaPratoSchema } from '../utils/categoriaPratoValidation'

type CategoriaPratoFormProps = {
  categoria: CategoriaPratoComContagem | null
  isSubmitting: boolean
  onCancel: () => void
  onSubmit: (values: CategoriaPratoFormValues) => Promise<void>
}

const defaultValues: CategoriaPratoFormValues = {
  nome: '',
  ordem_exibicao: 0,
  ativo: true,
}

export function CategoriaPratoForm({
  categoria,
  isSubmitting,
  onCancel,
  onSubmit,
}: CategoriaPratoFormProps) {
  const {
    register,
    handleSubmit,
    reset,
    formState: { errors },
  } = useForm<CategoriaPratoFormValues>({
    resolver: zodResolver(categoriaPratoSchema),
    defaultValues,
  })
      ...
      <section className="max-h-[92vh] w-full overflow-y-auto rounded-t border border-stone-200 bg-white p-5 shadow-xl sm:mx-auto sm:max-w-xl sm:rounded">
        <div className="mb-5 flex items-start justify-between gap-4">
          <div>
            <h3 className="text-lg font-semibold text-slate-950">
              {categoria ? 'Editar categoria de prato' : 'Nova categoria de prato'}
            </h3>
        ...
        <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
          ...
          <label className="block">
            ...
            />
            {errors.ordem_exibicao && (
              <span className="mt-1 block text-xs text-red-700">
                {errors.ordem_exibicao.message}
              </span>
            )}
          </label>
